...
public class Shelf{
    ...
    public static boolean addBook(int id, String author, String title, int pages ) {
        if (!isIDUnique(id) || title.length() == 0 || pages <= 0) {
            return false;
        } else {
     ...
     */
        public static boolean addBook(Book book ){
            if( !isIDUnique(book.getId())   || book.getTitle().length()==0 ||  book.getPages()<=0    ){
                return false;
            }else{
                theBooks.add(book);
                return true;
        ...
        public static Book findBookByPartialTitle(String title) {
            for (int x = 0; x < theBooks.size() ; x++) {
                if(theBooks.get(x).getTitle().toLowerCase().contains(title.toLowerCase())){
        ...
        public static List<Book> getAllBooksForAuthor(String author) {
            ...
            for (int x = 0; x < theBooks.size() ; x++) {
                if(theBooks.get(x).getAuthor().equalsIgnoreCase(author)){
                    allBooksFromAuthor.add(theBooks.get(x));
                }
            }
